...
const SAMPLE_REPORTS = [
  { id:"GBV-001", date:"2024-06-01", lga:"Gboko", type:"GBV", severity:"High", summary:"Domestic violence reported near market area. Victim needs shelter.", source:"Web", reviewStatus:"Not Reviewed", aiPriority:null, aiReasoning:"", culturalFlag:false, culturalNote:"" },
  { id:"ECO-001", date:"2024-06-03", lga:"Makurdi", type:"Economic Barrier", severity:"Medium", summary:"Women traders denied loans by cooperative. Financial exclusion persists.", source:"Web", reviewStatus:"Not Reviewed", aiPriority:null, aiReasoning:"", culturalFlag:false, culturalNote:"" },
  { id:"GBV-002", date:"2024-06-05", lga:"Otukpo", type:"GBV", severity:"Critical", summary:"Sexual assault case. Perpetrator is community leader. Fear of reporting.", source:"SMS", reviewStatus:"Not Reviewed", aiPriority:null, aiReasoning:"", culturalFlag:false, culturalNote:"" },
  { id:"ECO-002", date:"2024-06-07", lga:"Gboko", type:"Economic Barrier", severity:"Low", summary:"No market access for rural women due to poor road infrastructure.", source:"USSD", reviewStatus:"Not Reviewed", aiPriority:null, aiReasoning:"", culturalFlag:false, culturalNote:"" },
  { id:"GBV-003", date:"2024-06-09", lga:"Katsina-Ala", type:"GBV", severity:"High", summary:"Early marriage case. Girl aged 14. Family pressure involved.", source:"USSD", reviewStatus:"Not Reviewed", aiPriority:null, aiReasoning:"", culturalFlag:false, culturalNote:"" },
];
    ...
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:20 }}>
        <div style={{ background:"#fff", borderRadius:12, padding:18, boxShadow:"0 2px 8px #0001" }}>
          <h3 style={{ margin:"0 0 12px", color:"#1e1b4b", fontSize:15 }}>Recent Activity</h3>
          {reports.slice(-3).reverse().map(r=>(
            <div key={r.id} style={{ borderBottom:"1px solid #f0f0f0", padding:"8px 0", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div>
                ...
                <div style={{ color:"#555", fontSize:12, marginTop:2 }}>{r.summary.substring(0,60)}…</div>
              </div>
              <Badge label={r.severity} color={COLORS[r.severity]}/>
            </div>
          ))}
        </div>
        <div style={{ background:"#fff", borderRadius:12, padding:18, boxShadow:"0 2px 8px #0001" }}>
  ...
  return (
    <div>
      ...
      <div style={{ marginTop:28 }}>
        <h3 style={{ color:"#1e1b4b" }}>Report Database ({reports.length} records)</h3>
          ...
          <table style={{ width:"100%", borderCollapse:"collapse", background:"#fff", borderRadius:12, overflow:"hidden", boxShadow:"0 2px 8px #0001" }}>
            <thead>
              <tr style={{ background:"#f5f3ff" }}>
                {["ID","Date","Source","LGA","Type","Severity","Summary"].map(h=>(
                  <th key={h} style={{ padding:"10px 14px", textAlign:"left", fontSize:13, color:"#4338ca", fontWeight:700 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {reports.map((r,i)=>(
                <tr key={r.id} style={{ background: i%2?"#fafafa":"#fff", borderTop:"1px solid #f0f0f0" }}>
                  <td style={{ padding:"8px 14px", fontSize:13, fontWeight:700, color:"#6366f1" }}>{r.id}</td>
                  <td style={{ padding:"8px 14px", fontSize:13 }}>{r.date}</td>
                  ...
                  <td style={{ padding:"8px 14px", fontSize:12, color:"#555", maxWidth:260 }}>{r.summary.substring(0,80)}{r.summary.length>80?"…":""}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function AIAnalysis({ reports, setReports }) {
  ...
  useEffect(()=>{
    setText(reports.map(r=>`[${r.id}] ${r.lga} | ${r.type} | ${r.severity}: ${r.summary}`).join("\n"));
  },[reports]);
  ...
  const analyze = async () => {
    ...
    try {
      ...
      setResult(res);
    } catch(e) { setResult("Error calling AI. Please try again."); }
    setLoading(false);
  };
  ...
  return (
    <div>
      ...
      <p style={{ color:"#666" }}>NLP-powered pattern detection and per-report priority classification, feeding a human-reviewed queue.</p>
          ...
          {lgaCounts.map(({lga,count})=>(
            <div key={lga} style={{ marginBottom:10 }}>
              ...
              <div style={{ background:"#e0e7ff", borderRadius:4, height:10 }}>
                <div style={{ background:"#6366f1", borderRadius:4, height:10, width:`${(count/reports.length)*100}%` }}/>
              </div>
            </div>
          ))}
        </div>
            ...
            return (
              <div key={s} style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
                ...
                <div style={{ flex:1, background:"#f3f4f6", borderRadius:4, height:18 }}>
                  <div style={{ background:COLORS[s], borderRadius:4, height:18, width:`${(cnt/reports.length)*100}%`, transition:"width 0.4s" }}/>
                </div>
      ...
      {result && (
        <div style={{ background:"#fff", borderRadius:12, padding:24, boxShadow:"0 2px 8px #0001", borderLeft:"4px solid #6366f1" }}>
          <h3 style={{ margin:"0 0 12px", color:"#1e1b4b" }}>AI Intelligence Report (advocacy-level, not case-level)</h3>
...
function AdminPortal({ reports, setReports }) {
  ...
  const sections = ["Review Queue","Reports","Staff","Analytics","Settings"];
  const staff = [
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:20, flexWrap:"wrap" }}>
        {sections.map(s=>(
          <button key={s} onClick={()=>setSection(s)} style={{ padding:"8px 18px", borderRadius:8, border:"none", cursor:"pointer", background: section===s?"#6366f1":"#e0e7ff", color: section===s?"#fff":"#4338ca", fontWeight:600, position:"relative" }}>
            {s}{s==="Review Queue" && pending.length>0 && <span style={{ marginLeft:6, background:"#dc2626", color:"#fff", borderRadius:10, padding:"1px 7px", fontSize:11 }}>{pending.length}</span>}
          </button>
        ))}
      </div>
      ...
      {section==="Reports" && (
        <div>
          <h3 style={{ color:"#1e1b4b" }}>All Reports ({reports.length})</h3>
          <table style={{ width:"100%", borderCollapse:"collapse", background:"#fff", borderRadius:12, overflow:"hidden", boxShadow:"0 2px 8px #0001" }}>
            <thead>
              <tr style={{ background:"#f5f3ff" }}>
